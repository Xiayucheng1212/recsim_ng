...
class UserWithInterestedTopics(object):
  ...
  def initial_state(self):
    ...
    for i in range(self._num_users):
      ...
      interested_topics[np.random.choice(self._num_topics, interested_topics_num, replace=False)] = 4.0
    ...
    interest_initial_state = Value(state = tf.random.normal(shape=[self._num_users, self._num_topics] ,mean=tf.convert_to_tensor(all_users_interested_topics), stddev=0.2*tf.ones([self._num_users, self._num_topics])))
  ...
  def next_state(self, previous_state, user_response,
    ...
    direction = tf.expand_dims(
        chosen_doc_features.get('doc_quality'), axis=-1) * (
            doc_vector - user_interests) * 0.9 # Notice: 0.5 is the step size
